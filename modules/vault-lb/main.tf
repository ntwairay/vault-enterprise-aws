...
#   subnet_ids = aws_network_interface.multi_ips.*.id
#   eips       = aws_eip.multi_eips.*.allocation_id
# }
#
...
# }
#
# resource "aws_network_interface" "multi_ips" {
#   count       = length(var.subnet_mapping)
#
#   subnet_id   = lookup(var.subnet_mapping[count.index], "subnet_id")
#   private_ips = [lookup(var.subnet_mapping[count.index], "private_ip")]
# }
#
# resource "aws_eip" "multi_eips" {
#   count                     = length(aws_network_interface.multi_ips.*.id)
#
#   vpc                       = true
#   network_interface         = element(aws_network_interface.multi_ips.*.id, count.index)
#   associate_with_private_ip = lookup(var.subnet_mapping[count.index], "private_ip")
# }
